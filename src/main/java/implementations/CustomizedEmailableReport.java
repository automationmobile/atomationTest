...
public class CustomizedEmailableReport implements IReporter
{
	...
	protected void generateMethodSummaryReport(List<ISuite> suites)
	{
		...
		for (ISuite suite : suites)
		{
			...
			for (ISuiteResult r2 : r.values())
			{
				...
				String testName = testContext.getSuite().getName();
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details)
	{
		if (tests.getAllResults().size() > 0)
		{
			...
			for (ITestNGMethod method : getMethodSet(tests, suite))
			{
				...
				{})[0].getTestName();
				buff.append("<td><a href=\"#m" + methodIndex + "\">" + qualifiedName(method) + " " + (description != null && description.length() > 0 ? "(\"" + description + "\")" : "") + "</a>" + (null == testInstanceName ? "" : "<br>(" + testInstanceName + ")") + "</td>"
						+ "<td class=\"numi\" style=\"color:white;text-align:left;padding-center:2em\">" + firstLine + "<br/></td>" + "</tr>");
				// resultSet.size();
			}
			if (mq > 0)
			{
				cq += 1;
				out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td style=" + "color:white;");
				if (mq > 1)
				{
					out.print(" rowspan=\"" + mq + "\"");
				}
	...
	private void startResultSummaryTable(String style)
	{
		...
		out.println("<tr><th>Class</th>" + "<th>Method</th><th>Exception</th></tr>");
		row = 0;
	}
	...
	private String qualifiedName(ITestNGMethod method)
	{
		...
		String color = "style=" + "color:white;text-decoration:underline;" + "";
		return "<b " + color + ">" + method.getMethodName() + "</b> " + addon;
	}
	...
	private void generateForResult(ITestResult ans, ITestNGMethod method, int resultSetSize)
	{
		...
		if (hasParameters)
		{
			...
			{
				out.println("<td>" + Utils.escapeHtml(Utils.toString(p, null)) + "</td>");
			}
			out.println("</tr>");
		}
	...
	private void getParameters(IResultMap tests)
	{
		for (ITestResult result : tests.getAllResults())
		{
			...
			if (hasParameters)
			{
				for (Object p : parameters)
				{
					out.println(Utils.escapeHtml(Utils.toString(p, null)) + " | ");
				}
			}
		}
	}
	...
	public void generateSuiteSummaryReport(List<ISuite> suites)
	{
		...
		tableColumnStart("Total Execution<br/>Time(hh:mm:ss)");
		...
		for (ISuite suite : suites)
		{
			...
			for (ISuiteResult r : tests.values())
			{
				...
				// String ClassName="GiftFinderIndia";
				startSummaryRow(overview.getSuite().getName());
				...
				// time_start);
				// time_end = Math.max(overview.getEndDate().getTime(),
				// time_end);
				/*
				 * summaryCell(
				 ...
				 */
				summaryCell(millisToTimeConversion((overview.getEndDate().getTime() - overview.getStartDate().getTime()) / 1000), true);
	...
	{
		row += 1;
		out.print("<tr" + (row % 2 == 0 ? " class=\"stripe\"" : "") + "><td style=\"text-align:center;padding-center:2em\"><a href=\"#t" + testIndex + "\"><b>" + label + "</b></a>" + "</td>");
	}
	...
	{
		out.println("");
		out.println("<table cellspacing=\"0\" cellpadding=\"0\"" + (cssclass != null ? " class=\"" + cssclass + "\"" : " style=\"padding-bottom:2em\"") + (id != null ? " id=\"" + id + "\"" : "") + ">");
		row = 0;
	}
	...
	protected void startHtml(PrintWriter out)
	{
		...
		out.println(".param th {padding-left:1em;padding-center:1em}");
		out.println(".param td {padding-left:.5em;padding-center:2em}");
		out.println(".stripe td,.stripe th {background-color: #E6EBF9}");
		out.println(".numi,.numi_attn {text-align:center}");
		out.println(".total td {font-weight:bold}");
		out.println(".passedodd td {background-color: #0A0}");
		out.println(".passedeven td {background-color: #3F3}");
		out.println(".skippedodd td {background-color: #b6b2b2}");
		out.println(".skippedodd td {background-color: #b6b2b2}");
	...
	private class TestSorter implements Comparator<IInvokedMethod>
	{
		...
		{
			// System.out.println("Comparing " + ((ITestNGMethod)
			// o1).getMethodName() + " " + o1.getDate() + " and " +
	...
	private class TestResultsSorter implements Comparator<ITestResult>
	{
		...
		{
			// return (int) (o1.getMethod().getDate() -
			// o2.getMethod().getDate());
			// System.out.println("First method class name
			// "+o1.getTestClass().getName());
			// System.out.println("second method class name
			// "+o2.getTestClass().getName());
			...
			if (result == 0)
			{
				...
				// "+o1.getMethod().getMethodName());
				// System.out.println("second method name
				// "+o2.getMethod().getMethodName());
